...
func stddev(numbers []float64, mean float64) float64 {
    sum_square := 0.0
    for _, x := range numbers {
        sum_square += math.Pow(x - mean, 2)
    }
    return math.Sqrt(sum_square / float64(len(numbers) - 1.0))
}

func mode(numbers []float64) []float64 {
    numFreq := make(map[float64] int)
    maxFreq := 0
    modes := make([]float64, 0)
    for _, x := range numbers {
        numFreq[x]++
        if numFreq[x] > maxFreq {
            maxFreq = numFreq[x]
        }
    }
