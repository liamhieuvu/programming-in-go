...
<body><h3>Soundex</h3><p>Computes soundex codes for a list of names</p>`
...
func homePage(writer http.ResponseWriter, request *http.Request) {
    ...
    if err != nil {
        ...
        fmt.Fprint(writer, processRequest(request))
    }
    fmt.Fprint(writer, pageBottom)
}

func processRequest(request *http.Request) string {
    if slice, found := request.Form["names"]; found && len(slice) > 0 {
        text := strings.Replace(slice[0], ",", " ", -1)
        table := `<table border="1"><tr><th>Name</th><th>Soundex</th></tr>`
        for _, field := range strings.Fields(text) {
            table += "<tr><td>" + html.EscapeString(field) + "</td><td>" +
                html.EscapeString(soundex(field)) + "</td></tr>"
        }
        return table + "</table>"
    }
    return ""
