...
func readPlsPlaylist(data string) (songs []Song) {
    ...
    for _, line := range strings.Split(data, "\n") {
        ...
        if strings.HasPrefix(fields[0], "File") {
            song.Filename = strings.Map(mapPlatformDirSeparator, fields[1])
        } else if strings.HasPrefix(fields[0], "Title") {
            song.Title = fields[1]
        } else if strings.HasPrefix(fields[0], "Length") {
            var err error
            if song.Seconds, err = strconv.Atoi(fields[1]); err != nil {
                song.Seconds = -1
            }
        }
        if song.Filename != "" && song.Title != "" && song.Seconds != 0 {
